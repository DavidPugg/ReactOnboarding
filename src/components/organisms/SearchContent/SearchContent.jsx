...
import SearchMovieItem from "../../molecules/SearchMovieItem/SearchMovieItem"; //for movies, shows and companies
  ...
  return [
      ...
      <div className={c.list}>
        ...
        {keywords.length > 0 &&
          keywords.map(({ id, name }) => (
            <SearchKeywordItem key={id} name={name} />
          ))}
      </div>
    ),
    ...
    type == "network" && (
      <div className={c.list}>
        ...
        {/* {networks.length > 0 &&
          networks.map(({ id, poster_path, title, release_date }) => (
            <SearchMovieItem
              key={id}
              img={poster_path}
              title={title}
